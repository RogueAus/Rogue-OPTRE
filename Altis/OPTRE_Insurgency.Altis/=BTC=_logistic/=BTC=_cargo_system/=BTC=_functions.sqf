...
BTC_l_load =
{
	...
	if (count _this isEqualTo 0) then {_array = nearestObjects [player, BTC_def_vehicles, 5];} else {_array = _this;};
...
BTC_l_unload =
{
	...
	if (_id != -1) then
	{
		...
		_obj setpos (_veh modelToWorld [0,-9,-0.7]);
		switch (true) do
		{
			case (_height >= 20):
			{
...
BTC_l_placement =
{
	...
	BTC_l_plac_obj allowdamage false;
	...
	BTC_l_plac_obj allowdamage true;
...
BTC_l_keydown =
{
	...
	if (count _this > 1) then
	{
		...
		switch (true) do
		{
			...
			case (_key isEqualTo 32 && _alt && !_ctrl) : {_dir = getdir BTC_l_plac_obj;BTC_l_plac_obj setDir (_dir + 1);};

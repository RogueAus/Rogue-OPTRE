...
if (!_cache) then {
	...
	if ismultiplayer then {
		...
		if (_heightLimit) then {
			_actCond="{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 10} count allUnits > 0";
		}else{
...
waituntil {triggeractivated _eosActivated && RedHot <= Max_Act_Gzones};	//WAIT UNTIL PLAYERS IN ZONE and count of activated zones less than or equal to limiter

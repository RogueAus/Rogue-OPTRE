...
if (Airfield_opt) then
{
	if ((INS_op_faction isEqualTo 9) && (isClass(configFile >> "cfgPatches" >> "SUD_A10"))) then
	{
		...
		if (((INS_op_faction isEqualTo 10) || (INS_op_faction isEqualTo 11)) && (isClass(configFile >> "CfgVehicles" >> "RHS_A10"))) then
		{
			_dirfw1 = getDir INS_fw_1;
	...
	[] spawn {
		if (!(getMarkerColor "AircraftMaintenance" isEqualTo "") || (markerAlpha "AircraftMaintenance" isEqualTo 1)) then {
			...
			_onActiv="
				...
				{_x setDamage 0; _x setFuel 0; _x setVehicleAmmo 1;} count _uavArr;
			";
